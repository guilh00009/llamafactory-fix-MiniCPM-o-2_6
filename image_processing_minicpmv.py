...
class MiniCPMVImageProcessor(BaseImageProcessor):
    ...
    def __init__(self, max_slice_nums=9, scale_resolution=448, patch_size=14, **kwargs):
        ...
        # keep mean/std as numpy arrays but allow arbitrary shapes coming from external sources
    ...
    def _safe_normalize(self, image: np.ndarray, mean: np.ndarray, std: np.ndarray) -> np.ndarray:
        """
        Normalize `image` with `mean` and `std` safely handling different shapes that may appear in cached modules.
        Accepts images in channel-last (H,W,C) or channel-first (C,H,W).
        """
        ...
        # Channel-last: (H, W, C)
        if image.ndim == 3 and image.shape[-1] == mean.shape[0]:
            return (image - mean[None, None, :]) / std[None, None, :]
        # Channel-first: (C, H, W)
        if image.ndim == 3 and image.shape[0] == mean.shape[0]:
            return (image - mean[:, None, None]) / std[:, None, None]

        # Fallback: try to reshape mean/std to (1,1,C)
        try:
            mean_r = mean.reshape((1, 1, -1)) if mean.size > 1 else mean
            std_r = std.reshape((1, 1, -1)) if std.size > 1 else std
            return (image - mean_r) / std_r
        except Exception:
            # Last resort: elementwise broadcast (may fail but we keep it as last option)
        ...
        **kwargs,
        ...
        for _images in images_list:
            ...
            for image in _images:
                ...
                image_patches = [to_numpy_array(img).astype(np.float32) / 255.0 for img in image_patches]

                # prepare mean/std as 1D arrays once
                mean = np.asarray(self.mean).reshape(-1)
                std = np.asarray(self.std).reshape(-1)

                # normalize each patch safely
                image_patches = [self._safe_normalize(img, mean, std) for img in image_patches]

                # ensure channel-first for downstream reshape_by_patch
                image_patches = [
                    to_channel_dimension_format(img, ChannelDimension.FIRST, input_channel_dim=input_data_format)
